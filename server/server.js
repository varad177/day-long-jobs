...
server.post("/create-post", verifyJWT, (req, res) => {
  ...
  if (!mobileNumber) {
    return res.status(400).json({
      error: "You must provide mobile number",
    });
  }
  if (!numberOfDays) {
    return res.status(400).json({
      error: "You must provide number of days",
    });
  }
  if (!jobType) {
    return res.status(400).json({
      error: "You must provide job type",
    });
  }
  if (!email) {
    return res.status(400).json({
  ...
  if (!locationLink) {
    return res.status(400).json({
      error: "You must provide location link",
    });
  }
  if (!banner.length) {
    return res.status(400).json({
...
server.post("/latest-post", (req, res) => {
  let { page } = req.body;
  ...
  Blog.find({ draft: false })
    ...
    .sort({ publishedAt: -1 })
    .select(
      "blog_id title des banner activity city jobType city numberOfDays  publishedAt -_id "
    )
    .then((blogs) => {
      return res.status(200).json({
        blogs,
...
server.post("/search-post", (req, res) => {
  let { city, query, page, author } = req.body;

  let findQuery;

  if (city) {
    findQuery = { city: city, draft: false };
  } else if (query) {
...
server.post("/get-post", (req, res) => {
  ...
  Blog.findOneAndUpdate(
    ...
    .select(
      "title _id email lat long mobileNumber numberOfDays jobType banner locationLink city des activity comments"
    )
    .then((blog) => {
      User.findOneAndUpdate(
