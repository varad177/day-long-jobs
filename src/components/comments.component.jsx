...
const CommentContainer = () => {
    ...
    let {  commentWrapper, setCommentWrapper , post } = useContext(BlogContext)
    ...
    return (
        <div className={"max-sm:w-full fixed z-50 " + (commentWrapper ? "top-0 sm: right-[0] " : " top-[100%] sm:right-[-100%] ") + " duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full bg-white shadow-xl p-8 px-10 overflow-y-auto overflow-x-hidden "}>

            <div className="relative mt-4">
                <h1 className='text-xl font-medium '>Comments</h1>
                <p className="text-lg text-dark-grey line-clamp-1">{post.title}</p>
            ...
            {
              post.comments.results && post.comments.results.length ? post.comments.results.map((comment, i) => {
